...
const ViewAssignStaff = () => {
    ...
    const loadStaff = async () => {
        const result = await axios.get("http://localhost:9090/assignStaffEventsS");
        setStaff(result.data);
    }

    const deleteStaff = async (id) => {
        await axios.delete(`http://localhost:9090/assignStaffEvents/${id}`)
        loadStaff()
    }
        ...
        <>
         ...
         {/* </div> */}
              ...
              <div class="details" style={{ width: '1850px', height: 'auto' }}>
                ...
                <div className='py-4'>
                <h1>ASSIGN STAFF EACH EVENTS DETAILS</h1>
                <br/><br/><br/><br/>
                <table className="table border shadow">
                    ...
                    <tbody>
                        ...
                        {
                            assignStaffEvents.map((assignStaffEvents, index) => (
                                <tr>
                                    <th scope="row" key={index}>{index + 1}</th>
                                    <td>{assignStaffEvents.staffName}</td>
                                    <td>{assignStaffEvents.date}</td>
                                    <td>{assignStaffEvents.time}</td>
                                    <td>{assignStaffEvents.venue}</td>
                                    <td>{assignStaffEvents.duty}</td>
                                    <td>
                                        <Link className='btn btn-primary mx-2'to={`/viewassignstaffdetails/${assignStaffEvents.id}`}>View</Link>
                                        <Link className='btn btn-outline-primary mx-2' to={`/editassignstaff/${assignStaffEvents.id}`}>Update</Link>
                                        <button className='btn btn-danger mx-2' onClick={() => deleteStaff(assignStaffEvents.id)}>Delete</button>
                                    </td>
                                </tr>
    ...
    export default ViewAssignStaff